...
export function EntryDrawerForm({
  mode = 'create',
  defaultValues,
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<EntryFormValues>({
    ...
    defaultValues: {
      ...
      quantity: '',
      donorId: preselectedPerson?.id ?? null,
      notes: '',
      ...defaultValues,
    },
  })
  ...
  function handlePersonSelect(person: Person) {
    ...
    setValue('donorId', person.id, { shouldValidate: true })
    setSelectorOpen(null)
  }
  ...
  return (
    <>
      <div className="scrollarea" style={{ overflowY: 'auto', padding: '6px 22px 30px' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <label style={LABEL_STYLE}>
            Donante <span style={OPTIONAL_STYLE}>· opcional</span>
          </label>
          <button
            type="button"
          ...
          >
            <span style={{ fontSize: 15, fontWeight: 600, color: selectedPerson ? '#12212e' : '#9aa8b6' }}>
              {selectedPerson ? `${selectedPerson.names} ${selectedPerson.surnames}` : 'Seleccionar donante…'}
            </span>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#9aa8b6" strokeWidth="2.2" strokeLinecap="round">
              <path d="m6 9 6 6 6-6"/>
            </svg>
      ...
      {/* Person selector sub-drawer */}
      <SelectorDrawer
        open={selectorOpen === 'person'}
        title="Seleccionar donante"
